...
def generar_info_productos(productos: list) -> str:
    ...
    for i, prod in enumerate(productos):
        ...
        # Formatear precio en guaraníes con separadores de miles y símbolo GS
        precio_gs = format_guaranies(prod['precio_gs'])
        ...
        texto.append(
            f"📌 **{prod['nombre']}**\n"
            f"   💵 Precio: {precio_gs} (${prod['precio_usd']:.2f} USD)\n"  # Cambiado el orden y formato
            f"   🎨 Colores: {', '.join(prod['colores'])}\n"
            f"   💾 Almacenamiento: {', '.join(prod.get('almacenamiento', ['N/D']))}\n"
            f"{detalles_extra}"
            f"   ⚙️ Destacado: {'; '.join(prod['caracteristicas'][:3])}..."
        )
    return "\n\n".join(texto)

def format_guaranies(amount: int) -> str:
    """Formatea un número como precio en guaraníes con separadores de miles."""
...
def buscar_por_caracteristicas(pregunta_lower: str, caracteristicas_solicitadas: set, catalogo: dict) -> str:
        ...
        for prod in productos:
            ...
            if mencionadas_en_pregunta and any(caract in texto_producto for caract in caracteristicas_solicitadas):
                if any(caract in texto_producto for caract in caracteristicas_solicitadas if caract in pregunta_lower):
                    resultados_productos.append(prod)
    ...
    if resultados_unicos:
        texto_resultados = []
        for prod in resultados_unicos[:5]:
            precio_gs = format_guaranies(prod['precio_gs'])
            categoria_prod = next((cat for cat, prods in catalogo.items() if prod in prods), "Desconocida")
            texto_resultados.append(
                f"• **{prod['nombre']}** ({categoria_prod.capitalize()}) - Precio: {precio_gs} (${prod['precio_usd']:.2f} USD). "
                f"*Características clave*: {', '.join(prod['caracteristicas'][:2])}..."
            )
